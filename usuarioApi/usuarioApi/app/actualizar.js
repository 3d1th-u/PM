...
export default function ActualizarUsuarioScreen() {
  ...
  // Precargamos el estado con los datos que llegaron por la URL
    ...
    try {
      ...
      const respuesta = await fetch(`http://172.20.10.3:5000/v1/usuarios/${params.id}`, {
        ...
        headers: { 
          ...
          // Agregamos la autorización básica con el usuario y password en Base64
      ...
      if (respuesta.ok) {
        ...
        mostrarMensaje('Error', `No se pudo actualizar. El servidor respondió: ${respuesta.status}`);
      }
    } catch (error) {
